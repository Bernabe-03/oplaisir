...
export class OrdersService {
  ...
  // Décrémente le stock à partir des items d'une commande (utilisé lors de la validation)
  private async decrementStockForOrder(orderItems: any[]) {
    console.log('🚚 [decrementStockForOrder] Début de la décrémentation...');
    for (const item of orderItems) {
      ...
      if (item.type === 'product') {
        ...
        if (!productId) {
          console.error(`❌ Produit ${item.id} n'a pas de productId`);
          throw new BadRequestException(`Produit ${item.name} (ID: ${item.id}) : productId manquant`);
        }
        const product = await this.prisma.product.findUnique({ where: { id: productId } });
        if (!product) throw new BadRequestException(`Produit ${item.name} (ID: ${productId}) non trouvé`);
        if (product.stock < item.quantity) {
          throw new BadRequestException(
            `Stock insuffisant pour le produit ${item.name}. Disponible: ${product.stock}, Demandé: ${item.quantity}`
          );
        ...
        console.log(`📦 Stock produit ${productId} décrémenté de ${item.quantity}`);
      } 
      else if (item.type === 'support') {
        const supportId = item.supportId;
        if (!supportId) {
          console.error(`❌ Support ${item.id} n'a pas de supportId`);
          throw new BadRequestException(`Support ${item.name} (ID: ${item.id}) : supportId manquant`);
        }
        const support = await this.prisma.support.findUnique({ where: { id: supportId } });
        if (!support) throw new BadRequestException(`Support ${item.name} (ID: ${supportId}) non trouvé`);
        if (support.stock === 999) {
          console.log(`⚠️ Support ${supportId} stock illimité, ignoré`);
          continue;
        }
        if (support.stock < item.quantity) {
          throw new BadRequestException(
        ...
        console.log(`📦 Stock support ${supportId} décrémenté de ${item.quantity}`);
      } 
      else if (item.type === 'coffret') {
        const coffretId = item.coffretId;
        if (!coffretId) {
          console.error(`❌ Coffret ${item.id} n'a pas de coffretId`);
          throw new BadRequestException(`Coffret ${item.name} (ID: ${item.id}) : coffretId manquant`);
        }
        const coffret = await this.prisma.coffret.findUnique({ where: { id: coffretId } });
        if (!coffret) throw new BadRequestException(`Coffret ${item.name} (ID: ${coffretId}) non trouvé`);
        if (coffret.stock < item.quantity) {
          throw new BadRequestException(
            `Stock insuffisant pour le coffret ${item.name}. Disponible: ${coffret.stock}, Demandé: ${item.quantity}`
          );
        ...
        console.log(`📦 Stock coffret ${coffretId} décrémenté de ${item.quantity}`);
      }
    }
    console.log('✅ [decrementStockForOrder] Fin de la décrémentation');
  }
  ...
  async createOrder(createOrderDto: CreateOrderDto, userId?: string) {
    ...
    // 2. Validation préalable (stock et existence) – uniquement pour vérifier, pas de décrémentation
    for (const item of createOrderDto.items) {
      if (!item.id || !item.type) {
        throw new BadRequestException(`Item invalide : ${JSON.stringify(item)}`);
      }
    ...
    // 3. Création de la commande (sans décrémentation)
    try {
      ...
      console.log(`✅ Commande ${orderNumber} créée avec succès. Stock non décrémenté (sera fait à la validation).`);
  ...
  async validateOrder(id: string, validateOrderDto: ValidateOrderDto, userId: string) {
    ...
    switch (validateOrderDto.action) {
      case 'validate':
        ...
        console.log(`📉 Validation de la commande ${id}, décrémentation du stock...`);
        await this.decrementStockForOrder(order.items);
        break;
      case 'reject':
        newStatus = 'REJECTED';
        action = 'rejected';
