...
export class CoffretsController {
  ...
  updateStock(@Param('id') id: string, @Body() body: { quantity: number }) {
    return this.coffretsService.updateStock(id, body.quantity);
  }
  ...
  async generateBarcode(@Param('id') id: string, @Res() res: Response) {
    ...
    // TODO: Implémenter la génération de code-barre avec une librairie comme bwip-js
  ...
  async generateLabel(
    ...
    if (format === 'pdf') {
      // Correction PDFDocument: utilisation d'un cast pour éviter l'erreur de constructeur TS
      ...
      // En-tête (CORRIGÉ : doc.x, doc.y ajoutés pour lever l'ambiguïté de surcharge)
      ...
      // Code-barre factice (CORRIGÉ : doc.x, doc.y ajoutés)
      doc.moveDown(2);
